...
- (void)drawRect:(CGRect)rect
{
    ...
    if([self.dataSource respondsToSelector:@selector(drawBackgroundInRect:forDate:selected:)]){
        [self.dataSource drawBackgroundInRect:rect forDate:[self.date NSDate] selected:self.selected];
    }
    
    if (flags.marked){
        CGRect markerRect = CGRectMake(2.f,_tileSize.height-11.f ,_tileSize.width-4.f,7.f);
        if (self.dataSource == nil || ![self.dataSource drawMarkerInRect:markerRect forDate:[self.date NSDate] selected:self.selected]) {
    ...
    if([self.dataSource respondsToSelector:@selector(drawForegroundInRect:forDate:selected:)]){
        [self.dataSource drawForegroundInRect:rect forDate:[self.date NSDate] selected:self.selected];
    }
